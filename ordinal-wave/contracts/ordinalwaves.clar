...
(define-non-fungible-token ordinalwaves uint)
...
(define-read-only (get-owner (token-id uint))
	(ok (nft-get-owner? ordinalwaves token-id))
)

(define-public (transfer (token-id uint) (sender principal) (recipient principal))
	(begin
		(asserts! (is-eq tx-sender sender) err-not-token-owner)
		(nft-transfer? ordinalwaves token-id sender recipient)
	)
)
...
(define-public (mint (recipient principal))
	(let ((token-id (+ (var-get token-id-nonce) u1)))
		...
		(try! (nft-mint? ordinalwaves token-id recipient))
